...
void	init_segment(t_segment& seg, Point const p1, Point const p2) {
	seg.p1 = p1;
	seg.p2 = p2;
	seg.slope = (seg.p1.getY().toFloat() - seg.p2.getY().toFloat())
		/ (seg.p1.getX().toFloat() - seg.p2.getX().toFloat());
...
void	stretch_segment(t_segment& seg, Point& const opposite) {
	...
	while (seg.p1.getX().getRawBits() + i < opposite.getX().getRawBits()
		&& seg.p1.getY().getRawBits() + i < opposite.getY().getRawBits()
		&& seg.p2.getX().getRawBits() + i < opposite.getX().getRawBits()
		&& seg.p2.getY().getRawBits() + i < opposite.getY().getRawBits())
		i++; //
}
